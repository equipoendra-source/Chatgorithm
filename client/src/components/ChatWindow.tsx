...
export function ChatWindow({ socket, user, contact, config, onBack, onlineUsers, typingInfo, onOpenTemplates, quickReplies = [], currentAccountId, updateMyPreferences }: ChatWindowProps) {
    ...
    const updateCRM = (field: string, value: any) => { if (socket) { const updates: any = {}; updates[field] = value; if (field === 'assigned_to' && value && status === 'Nuevo') { updates.status = 'Abierto'; setStatus('Abierto'); } socket.emit('update_contact_info', { phone: contact.phone, updates: updates, actorUsername: user.username }); } };
    ...
    const handleAssign = (target: 'me' | string) => { if (!socket) return; const updates: any = { status: 'Abierto' }; if (target === 'me') { updates.assigned_to = user.username; setAssignedTo(user.username); } else { updates.department = target; updates.assigned_to = null; setAssignedTo(''); setDepartment(target); } socket.emit('update_contact_info', { phone: contact.phone, updates, actorUsername: user.username }); setStatus('Abierto'); setShowAssignMenu(false); };
                ...
                <div className={`border-b p-3 flex gap-2 md:flex-wrap md:gap-3 items-center shadow-sm z-10 shrink-0 ${isDark ? 'bg-slate-900/40 backdrop-blur-md border-white/5' : 'bg-white border-gray-200'}`} onClick={(e) => e.stopPropagation()}>
                    ...
                    {status === 'Nuevo' ? (
                        ...
                        <div className="hidden md:flex md:flex-wrap md:gap-3 md:items-center">
                            ...
                            <button
                                onClick={() => { const next = !aiMuted; setAiMuted(next); updateCRM('ai_muted', next); }}
                                title={aiMuted ? 'Laura silenciada para este cliente — pulsa para activarla' : 'Silenciar Laura para este cliente (no responderá más)'}
                                className={`flex items-center gap-1.5 px-2 py-1.5 rounded-md text-xs font-bold border transition ${aiMuted
                                    ? 'bg-rose-50 border-rose-200 text-rose-700 hover:bg-rose-100'
                                    : 'bg-emerald-50 border-emerald-200 text-emerald-700 hover:bg-emerald-100'}`}
                            >
                                {aiMuted ? <VolumeX className="w-3.5 h-3.5" /> : <Bot className="w-3.5 h-3.5" />}
                                ...
                                <>
                                    ...
                                    <div>
                                        ...
                                        >
                                            ...
                                            {department && !config?.departments?.includes(department) && (
                                                <option value={department}>{department} (antiguo)</option>
                                            )}
                                        </select>
                                    </div>
